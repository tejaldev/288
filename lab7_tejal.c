...
void goToObject(int angle, int dist, oi_t* sensor, int x_pos, int y_pos);
...
//            char str[20];
//                sprintf(str, "\n\r%d, %f", smallestObject.middle, smallestObject.distance);
//                printStringToPutty(str);

            if(switchManualAuto == 0)
                goToObject(smallestObject.middle, smallestObject.distance, sensor_data, x_pos, y_pos);
            else {
...
//    char str[20];
//    float des_x = cos(angle/180*3.14) * dist;
//    float des_y = sin(angle/180*3.14) * dist;
//    sprintf(str, "\n\r%f, %f", des_x, des_y);
//    printStringToPutty(str);

    if (angleFrom90 > 0) {
        //turn_clockwise(sensor, max(angleFrom90, 16));
        autoMoveForward(sensor, angle, max(dist, 5), x_pos, y_pos);
    }
    else {
